"""Compare manual SEGM with G2EGM and NEGM for 1 period - IDENTICAL defaults"""
...
print("="*70)
print("1-PERIOD COMPARISON: Manual SEGM vs G2EGM vs NEGM")
print("="*70)

# =============================================================================
# Solve all three methods with IDENTICAL defaults (T=1, only solver differs)
# =============================================================================
...
for method in ['G2EGM', 'NEGM']:
    print(f"\nSolving with {method}...")
    model = G2EGMModelClass(name=f'baseline')
    model.par.solmethod = method  # Only difference
    model.par.do_print = False
    # DON'T change T - use the default T=20 setup
    
    try:
        model.solve()
        
        t = 0
        # Check if there's a next period value function
        w_next = model.sol.w[t+1] if t+1 < len(model.sol.w) else None
        ...
        results[method] = {
            ...
            'w_next': w_next.copy() if w_next is not None else None,
        ...
        d = results[method]['d']
        print(f"  ✓ Solved")
        print(f"  T={model.par.T}, len(sol.c)={len(model.sol.c)}, len(sol.w)={len(model.sol.w)}")
        print(f"  d: [{d.min():.4f}, {d.max():.4f}], mean={d.mean():.4f}")
        print(f"  d>0.01: {np.sum(d > 0.01)} / {d.size}")
        
        # Check all w values
        for i in range(len(model.sol.w)):
            w_check = model.sol.w[i]
            print(f"  w[{i}]: min={w_check.min():.6f}, max={w_check.max():.6f}, nonzero={np.sum(np.abs(w_check) > 1e-10)}")
...
if results['G2EGM'] is None or results['NEGM'] is None:
    print("\n✗ Baseline models failed!")
    exit(1)

# Check agreement
diff_d = np.abs(results['G2EGM']['d'] - results['NEGM']['d'])
print(f"\nG2EGM vs NEGM: |Δd| max={diff_d.max():.6f}")

# =============================================================================
# Manual SEGM using EXACT same parameters
# =============================================================================

print(f"\n{'='*70}")
print("Manual SEGM (no numba)")
print(f"{'='*70}")
...
print(f"Post-decision value w[t=0]: [{w.min():.6f}, {w.max():.6f}], mean={w.mean():.6f}")
print(f"Non-zero w: {np.sum(np.abs(w) > 1e-10)} / {w.size}")

w_next = results['G2EGM']['w_next']
if w_next is not None:
    print(f"Post-decision value w[t=1]: [{w_next.min():.6f}, {w_next.max():.6f}], mean={w_next.mean():.6f}")
    print(f"Non-zero w_next: {np.sum(np.abs(w_next) > 1e-10)} / {w_next.size}")
    print(f"✓ Using w_next (retirement value) for continuation")
    w = w_next  # Use the NEXT period's post-decision value
else:
    print(f"✗ No w_next available!")
...
print(f"Computed wa: [{wa.min():.4f}, {wa.max():.4f}]")
print(f"Computed wb: [{wb.min():.4f}, {wb.max():.4f}]")

# Subproblem 1: Consumption (b, a) → (b, l)
print(f"\nSubproblem 1: Consumption...")
c_endo_all = []
l_endo_all = []
b_endo_all = []
w_endo_all = []
...
# Subproblem 2: Pension (b, l) → (n, m)
print(f"\nSubproblem 2: Pension...")
d_endo_all = []
c_endo_all2 = []
n_endo_all = []
m_endo_all = []
...
if len(d_endo_all) > 0:
    print(f"  d_endo: [{d_endo_all.min():.4f}, {d_endo_all.max():.4f}]")
    print(f"  d_endo>0.01: {np.sum(d_endo_all > 0.01)} / {len(d_endo_all)}")
else:
    print(f"  ✗ No valid endogenous points generated!")
...
print(f"  d_segm: [{d_segm.min():.4f}, {d_segm.max():.4f}]")
...
for method in ['NEGM', 'G2EGM']:
    ...
    if diff_c.max() < 0.01 and diff_d.max() < 0.01:
        ...
        print(f"  Worst: (n={grid_n[i_max[0]]:.2f}, m={grid_m[i_max[1]]:.2f})")
        print(f"    {method}: c={c_ref[i_max]:.4f}, d={d_ref[i_max]:.4f}")
